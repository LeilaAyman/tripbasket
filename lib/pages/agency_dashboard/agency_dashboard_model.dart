...
class _AgencyDashboardWidgetState extends State<AgencyDashboardWidget> {
  ...
  DocumentReference? _getCurrentAgencyRef() {
    ...
    if (userDoc == null) {
      print('DEBUG: No current user document found');
      return null;
    }
    
    final agencyRef = userDoc.agencyReference;
    if (agencyRef == null) {
      print('DEBUG: No agency reference found for user: ${user.uid}');
      // If no agency reference, show message (this shouldn't happen for agency users)
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No agency access found. Please contact support.'),
    ...
    print('DEBUG: Found agency reference: ${agencyRef.path}');
    return agencyRef;
  }
}
